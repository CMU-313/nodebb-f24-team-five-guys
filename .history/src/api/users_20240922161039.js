...
usersAPI.get = async (caller, { uid }) => {
	...
	if (userData) {
		userData.followerCount = await db.sortedSetScore('users:followers', uid) || 0;
	}
...
usersAPI.getMostPopular = async function (caller, data) {
	...
	// Ensure followerCount is included for each user
	for (const user of userData) {
		if (!user.followerCount) {
			user.followerCount = await db.sortedSetScore('users:followers', user.uid) || 0;
		}
	}
	return {
		users: userData,
...
usersAPI.getMostPopular.schema = {
	summary: 'Get users with most followers',
	parameters: [{
		name: 'page',
		in: 'query',
	...
	responses: {
		200: {
			description: 'Successfully retrieved users with most followers',
			content: {
				'application/json': {
					schema: {
						type: 'object',
